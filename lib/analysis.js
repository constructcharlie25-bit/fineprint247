...
function buildSystemPrompt(contractType) {
  ...
  const base = [
    ...
    'COVERAGE CHECK — before finalizing, verify you have considered every item in the applicable contract-type',
    '  checklist above (or the general WHAT TO LOOK FOR list when no type matches). For checklist items that are',
    '  fine as written, say nothing — but do not skip the check. Never finish a review having "forgotten" a',
    '  checklist category.',
    '',
